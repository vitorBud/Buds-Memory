export const focusStyles = {
  container: 'relative z-[1] flex h-dvh min-h-dvh w-full flex-col items-center justify-start overflow-x-hidden overflow-y-auto [-webkit-overflow-scrolling:touch] px-6 pt-[clamp(56px,6vh,86px)] pb-[76px] text-[var(--text)] [background:radial-gradient(circle_at_50%_30%,rgb(var(--accent-rgb)/0.16),transparent_50%),radial-gradient(circle_at_80%_80%,rgb(var(--accent-hot-rgb)/0.12),transparent_45%),var(--liquid-bg)] before:pointer-events-none before:fixed before:inset-0 before:z-0 before:bg-[linear-gradient(90deg,rgb(var(--accent-hot-rgb)/0.045)_1px,transparent_1px),linear-gradient(rgb(var(--accent-hot-rgb)/0.04)_1px,transparent_1px)] before:bg-[length:48px_48px] before:opacity-60 before:content-[""] max-[760px]:px-3.5 max-[760px]:pt-[calc(28px+env(safe-area-inset-top))] max-[760px]:pb-[calc(var(--mobile-nav-height)+32px+env(safe-area-inset-bottom))] supports-[-webkit-touch-callout:none]:max-[760px]:h-[-webkit-fill-available] supports-[-webkit-touch-callout:none]:max-[760px]:min-h-[-webkit-fill-available] platform-ios:before:!opacity-25',
  content: 'relative z-[2] mx-auto flex w-full max-w-[800px] flex-col gap-6 text-left max-[760px]:gap-4',
