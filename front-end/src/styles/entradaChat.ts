...
export const chatInputStyles = {
  ...
  modelButton:
    'inline-flex h-[34px] items-center gap-2 whitespace-nowrap rounded-aether border border-[rgb(var(--accent-rgb)/0.22)] bg-[rgb(12_8_4/0.78)] px-[9px] font-mono text-[11px] text-[var(--jarvis-muted)] transition-[background,border-color,color] duration-160 hover:border-[rgb(var(--accent-hot-rgb)/0.52)] hover:bg-[rgb(var(--accent-rgb)/0.13)] hover:text-[var(--jarvis-hot)] [&>span]:size-[7px] [&>span]:rounded-full [&>span]:bg-aether-accent-hot [&>span]:shadow-[0_0_18px_rgb(var(--accent-hot-rgb)/0.7)]',
  modelMenu:
    'absolute right-0 bottom-[calc(100%+8px)] z-20 min-w-[180px] overflow-hidden rounded-aether border border-[rgba(245,158,11,0.24)] bg-[rgba(8,6,4,0.96)] shadow-aether',
  modelOption:
    'grid w-full gap-0.5 border-0 bg-transparent px-2.5 py-[9px] text-left font-mono text-xs text-aether-muted hover:bg-[rgba(245,158,11,0.1)] hover:[&>span]:text-[var(--jarvis-hot)] [&>span]:font-sans [&>span]:text-xs [&>span]:font-bold [&>span]:text-aether-text [&>small]:text-[10px] [&>small]:text-aether-faint',
  modelOptionActive:
  ...
  meta:
    'mx-auto flex w-full max-w-[980px] justify-between px-0.5 text-[11px] text-aether-muted',
} as const
